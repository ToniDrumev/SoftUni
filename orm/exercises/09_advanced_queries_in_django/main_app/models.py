...
class VideoGame(models.Model):
    ...
    rating = models.DecimalField(
        ...
        validators=[
            ...
            validators.MaxValueValidator(10.0, message='The rating must be between 0.0 and 10.0'),
        ],
    )
...
class Invoice(models.Model):
    ...
    def get_invoice_with_billing_info(cls, invoice_number: str) -> object:
        return cls.objects.select_related('billing_info').filter(invoice_number=invoice_number)[0]
...
class Task(models.Model):
    ...
    def recent_completed_tasks(cls, days: int) -> QuerySet:
        return cls.objects.filter(
            Q(is_completed=True) & Q(completion_date__gte=F('creation_date') - days)
        )
