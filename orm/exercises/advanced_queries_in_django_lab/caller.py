...
def product_quantity_ordered():
    ordered_products = Product.objects.annotate(
        sum_quantity=Sum('orderproduct__quantity')
    ).filter(
        sum_quantity__gte=1
    ).order_by(
        '-sum_quantity'
    ...
    return "\n".join(f"Quantity ordered of {p.name}: {p.sum_quantity}" for p in ordered_products)


def ordered_products_per_customer():
    ordered_products = Product.objects.prefetch_related(
        orders='order_set',
