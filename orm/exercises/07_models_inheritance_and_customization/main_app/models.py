...
class RegularReservation(BaseReservation):
	def clean(self):
		...
		reservations = RegularReservation.objects.filter(
			room=self.room,
...
class SpecialReservation(BaseReservation):
	...
	def extend_reservation(self, days: int):
		existing_rooms = SpecialReservation.objects.filter(
			room=self.room,
			start_date__lte=self.end_date + timedelta(days),
			end_date__gte=self.start_date,
		)
